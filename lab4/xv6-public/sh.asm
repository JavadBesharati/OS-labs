  ...
  *ps = s;
  ...
  if(eq)
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     52e:	50                   	push   %eax
     52f:	68 44 1a 00 00       	push   $0x1a44
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     550:	56                   	push   %esi
     551:	68 4c 1a 00 00       	push   $0x1a4c
...
peek(char **ps, char *es, char *toks)
{
  ...
  while(s < es && strchr(whitespace, *s))
     ...
     5fd:	50                   	push   %eax
     5fe:	68 4c 1a 00 00       	push   $0x1a4c
...
00000f03 <sem_acquire>:
SYSCALL(sem_acquire)
     f03:	b8 1e 00 00 00       	mov    $0x1e,%eax
     f08:	cd 40                	int    $0x40
     f0a:	c3                   	ret    

00000f0b <sem_release>:
SYSCALL(sem_release)
     ...
     f5c:	89 d1                	mov    %edx,%ecx
{
  ...
  write(fd, &c, 1);
     ...
     fce:	56                   	push   %esi
     fcf:	57                   	push   %edi
     fd0:	e8 6e fe ff ff       	call   e43 <write>
  while(--i >= 0)
    ...
    1009:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
  ...
  write(fd, &c, 1);
    ...
    1045:	57                   	push   %edi
    1046:	56                   	push   %esi
    1047:	e8 f7 fd ff ff       	call   e43 <write>
  ...
  write(fd, &c, 1);
    ...
    109e:	e8 a0 fd ff ff       	call   e43 <write>
        putc(fd, c);
  ...
  write(fd, &c, 1);
    ...
    10af:	57                   	push   %edi
    10b0:	56                   	push   %esi
    10b1:	e8 8d fd ff ff       	call   e43 <write>
        putc(fd, c);
  ...
  write(fd, &c, 1);
    ...
    111b:	57                   	push   %edi
    111c:	56                   	push   %esi
    111d:	e8 21 fd ff ff       	call   e43 <write>
        while(*s != 0){
  ...
  write(fd, &c, 1);
    115f:	e8 df fc ff ff       	call   e43 <write>
        ap++;
    ...
    11a1:	a1 c4 1a 00 00       	mov    0x1ac4,%eax
{
  ...
  freep = p;
    ...
    11e1:	5b                   	pop    %ebx
  freep = p;
    11e2:	89 15 c4 1a 00 00    	mov    %edx,0x1ac4
}
    11e8:	5e                   	pop    %esi
    11e9:	5f                   	pop    %edi
    11ea:	5d                   	pop    %ebp
  ...
  freep = p;
    121c:	89 15 c4 1a 00 00    	mov    %edx,0x1ac4
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    123c:	8b 3d c4 1a 00 00    	mov    0x1ac4,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    1281:	8b 3d c4 1a 00 00    	mov    0x1ac4,%edi
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    1293:	e8 13 fc ff ff       	call   eab <sbrk>
  if(p == (char*)-1)
  ...
  return freep;
    12af:	8b 15 c4 1a 00 00    	mov    0x1ac4,%edx
    ...
    12f7:	1a 00 00 
    base.s.size = 0;
    12fa:	bf c8 1a 00 00       	mov    $0x1ac8,%edi
    base.s.ptr = freep = prevp = &base;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    base.s.size = 0;
    130b:	c7 05 cc 1a 00 00 00 	movl   $0x0,0x1acc
    1312:	00 00 00 
    if(p->s.size >= nunits){
